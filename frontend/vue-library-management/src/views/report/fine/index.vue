...
import { queryFineReasonCountApi, queryPayStatusCountApi } from "@/api/report.js";
import * as echarts from "echarts";


//前端定义对象存储职位数据
const reasonName = ref([]);
const reasonCount = ref([]);
...
//从后台获取罚款原因数据
const queryFineReasonData = async () => { 
  const result = await queryFineReasonCountApi();
  if(result.code == 200 && result.data){
    // 从后台获取罚款原因名称和数量
    reasonName.value = result.data.nameList;
    reasonCount.value = result.data.numList;
...
//从后台获取支付状态数据
const queryPayStatusData = async () => { 
  const result = await queryPayStatusCountApi();
  if(result.code == 200 && result.data){
    // 从后台获取支付状态名称和数量
    ...
    //修改为饼图需要的数据格式
    payStatusData.value = nameList.map((name, index) => {
      return {
        name: name,
        value: numList[index]
      };
...
const initBarChart = () => {
  ...
  const maxCount = reasonCount.value.length ? Math.max(...reasonCount.value) : 0;
...
const initPieChart = () => {
  ...
  const option = {
    ...
    series: [{
      type: 'pie',              // 饼状图类型
      radius: window.innerWidth < 768 ? '40%' : '35%', // 小屏幕增大饼图占比
      center: ['50%', '55%'],   // 饼图中心位置
      data: payStatusData.value,            // 数据
      itemStyle: {
        borderRadius: 5,        // 扇形圆角
